...
class Controller_Matches extends Controller_Application {
	...
	public function action_izmijeni()
	{
		...
		{
			if ($this->_user->cannot('*', $match->tournament))
			{
				$this->deny_access();
			}
			...
			if ($match->radiant_win !== NULL)
			{
				...
				$this->redirect('liga/turniri/'.$match->tournament->id.'-'.URL::title($match->tournament->name, '-', TRUE));
			}
			else
			{
				if ($this->request->method() === Request::POST)
				{
					try
					{
						...
						$this->redirect('liga/mecevi/'.$match->id);
					}
